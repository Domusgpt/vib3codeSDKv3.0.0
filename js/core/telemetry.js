...
function buildEvent(event, payload = {}) {
  ...
  return {
    ...
    meta: {
      version: payload.meta?.version || (hasDom ? 'browser' : 'headless'),
      userAgent: hasDom ? navigator.userAgent : 'node',
      ...payload.meta
    },
    error: payload.error
  };
...
const telemetry = hasDom ? new BrowserTelemetry() : noopTelemetry;
