/**
 * Exact Holographic Card Generator - MATCHES HolographicVisualizer EXACTLY
 * Uses the exact same shader code from src/holograms/HolographicVisualizer.js lines 151-451
 */

export class HolographicCardGeneratorExact {
    static generateCard(params, canvasId = 'trading-card-canvas') {
        ...
        // Use EXACT shader from HolographicVisualizer (lines 151-451)
...
<html>
<head>
    <title>VIB34D Holographic Trading Card - ${geometryName}</title>
    <style>
        body { 
            margin: 0; 
            padding: 20px; 
...
<body>
    ...
    <div class="info">
        <div>Holographic System - ${geometryName.charAt(0).toUpperCase() + geometryName.slice(1)}</div>
        <div>Density: ${params.gridDensity} | Morph: ${params.morphFactor}</div>
        <div>Hue: ${params.hue}° | Speed: ${params.speed}</div>
        <div>Enhanced Tetrahedron + Hypercube + Holographic Effects</div>
    </div>
    
    <script>
        // EXACT WebGL implementation matching HolographicVisualizer
        ...
        // EXACT parameters from function call (mapped like HolographicVisualizer)
        const mappedGridDensity = 0.6 + (parseFloat(${params.gridDensity || 15}) - 5) / 95 * 6.9; // Same 3x scaling
        
        // HSV to RGB conversion (same as used in HolographicVisualizer)
        ...
        const hue = ${params.hue || 320}; // Pink/magenta for holographic
        const [r, g, b] = hsvToRgb(hue, ${params.saturation || 0.8}, ${params.intensity || 0.6});
        
        const parameters = {
            geometry: ${params.geometry || 0},
            density: mappedGridDensity,
            speed: ${params.speed || 1.0},
            color: [r, g, b],
            intensity: ${params.intensity || 0.6},
            roleDensity: 1.0, // Content layer
            roleSpeed: 1.0,
            colorShift: ${params.hue || 320},
            chaosIntensity: ${params.chaos || 0.2},
            geometryType: ${params.geometry || 0},
            chaos: ${params.chaos || 0.2},
            morph: ${params.morphFactor || 1.0},
        ...
        function render() {
            ...
            // Set uniforms with EXACT values from HolographicVisualizer
            gl.uniform2f(uniforms.resolution, canvas.width, canvas.height);
            gl.uniform1f(uniforms.time, time);
            gl.uniform2f(uniforms.mouse, 0.5, 0.5);
            gl.uniform1f(uniforms.geometry, parameters.geometry);
            gl.uniform1f(uniforms.density, parameters.density);
            gl.uniform1f(uniforms.speed, parameters.speed);
            gl.uniform3fv(uniforms.color, parameters.color);
            gl.uniform1f(uniforms.intensity, parameters.intensity);
            gl.uniform1f(uniforms.roleDensity, parameters.roleDensity);
            gl.uniform1f(uniforms.roleSpeed, parameters.roleSpeed);
            gl.uniform1f(uniforms.colorShift, parameters.colorShift);
            gl.uniform1f(uniforms.chaosIntensity, parameters.chaosIntensity);
            ...
            gl.uniform1f(uniforms.geometryType, parameters.geometryType);
            gl.uniform1f(uniforms.chaos, parameters.chaos);
            gl.uniform1f(uniforms.morph, parameters.morph);
        ...
        return {
            filename: `vib34d-holographic-exact-${geometryName}-${timestamp}.html`,
            content: html,
            system: 'holographic'
        };
    }
