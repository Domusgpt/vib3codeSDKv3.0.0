...
## 2024-06-25 - Avoid array iteration methods in hot paths
**Learning:** Using `Array.prototype.forEach` or `Object.keys().forEach` in high-frequency methods like render loops creates unnecessary closure allocations and increases GC pressure, leading to frame drops.
**Action:** When working on render methods (like `_renderDirectFrame`) or high-frequency input handlers (like `updateParameter`, `resize`), always use standard `for` loops with pre-allocated indices or `for...in` loops to minimize memory allocations.
