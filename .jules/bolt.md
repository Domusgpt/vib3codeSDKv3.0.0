...
## 2024-05-31 - Optimized Object iteration via for...in loops
**Learning:** The continuous usage of `Object.entries(obj).forEach` and `Object.keys(obj).forEach` in core utilities like ParameterMapper causes significant closure allocation and garbage collection pressure in hot code paths. Benchmarks show replacing them with standard `for...in` loops (with `hasOwnProperty` checks) provides a ~34% to 80% performance boost by eliminating intermediate arrays and closure overheads.
**Action:** When iterating over parameter dictionaries in frequently called methods, prefer standard `for...in` loops caching keys using `Object.prototype.hasOwnProperty.call()` over allocating arrays via `Object.keys()` or `Object.entries()`.
