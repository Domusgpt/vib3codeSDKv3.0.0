...
## 2024-05-26 - [ParameterMapper Iteration Optimization]
**Learning:** Using `Object.entries(params).forEach(...)` is significantly slower than standard `for...in` loops (combined with `hasOwnProperty` checks) due to intermediate array allocations and closure overhead.
**Action:** When working in hot paths like parameter mapping, use `for...in` loops and cache static object lookups outside the loop for a ~50% speed increase. Ensure `return` statements in old callbacks are correctly updated to `continue`.
