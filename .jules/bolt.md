...
## 2024-05-26 - [Zero-Allocation Array Projection/Multiplication]
**Learning:** High-frequency rendering loops utilize array projection and matrix multiplication (`stereographicArray`, `orthographicArray`, `multiplyVec4Array`), but these methods were originally allocating a new array on every call via `.map()`, creating substantial GC pressure.
**Action:** Always provide an optional `target` parameter for array-based math operations. Bypassing `.map()` with a standard `for` loop and mutating pre-allocated output arrays yields measurable (~1.5x to ~2x) performance benefits.
