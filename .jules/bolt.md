...
## 2026-08-05 - ParameterMapper Object Iteration
**Learning:** Using `Object.entries(params).forEach(...)` in core mapping utilities like `ParameterMapper` causes significant intermediate array allocation and closure overhead during high-frequency execution paths.
**Action:** Replace `Object.entries().forEach()` and `Object.keys().forEach()` with `for...in` loops combined with `Object.prototype.hasOwnProperty.call()` for parameter objects to yield ~70% speedup and reduce GC pressure.
