...
## 2026-06-10 - [Equality and Identity Allocation Pressure]
**Learning:** Checking for equality (`equals`) or identity (`isIdentity`) on objects like `Mat4x4` and `Rotor4D` was inadvertently causing significant garbage collection pressure because they relied on `.identity()` which instantiates a new object on every call. Furthermore, looping or aggregate math checks in `equals` introduced unnecessary overhead.
**Action:** When implementing checks like `isIdentity()`, directly compare the properties or internal array against identity constants without instantiating new objects. Unroll loops for array checks and use short-circuit logic (early exits) to speed up `equals` and reduce computations.
