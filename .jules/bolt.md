...
## 2025-04-13 - [Zero-Allocation Geometry Warps]
**Learning:** In tight geometric hot loops in `HypertetraCore.js` and `HypersphereCore.js`, relying on `Array.prototype.map()` and implicit temporary `Vec4` allocations creates immense GC pressure, significantly degrading performance. Furthermore, optimizing geometric intersections (like `warpToEdges`) requires careful attention to the original logic: simply replacing `distanceTo` with `distanceToSquared` without re-verifying the distance comparison logic can inadvertently break edge snapping logic (as `e === 0 || distSq < nearestDistSq` checks can become impossible if state tracking is mutated prematurely). Lastly, vector class APIs must be strictly verified; for example, `src/math/Vec4.js` uses `lengthSquared()` and `distanceToSquared()`, not `lengthSq()` or `distanceToSq()`.
**Action:** Always pre-allocate arrays and target vectors (`target = null` parameter) for hot mathematical functions. Avoid using `.map()` inside frame or geometry-generation loops. When refactoring distance checks to use squared variants for performance, meticulously step through the assignment logic to ensure the comparison operators remain logically valid and don't prematurely short-circuit valid hits. Always double-check class method names using `grep` or `read_file` rather than assuming standard abbreviations.
