...
## 2024-05-26 - [Zero-Allocation Array Transformations]
**Learning:** Reusing the `.map()` optimization strategy from core math functions, methods that perform bulk array transformations like `Mat4x4.multiplyVec4Array` or `Projection.stereographicArray` generate significant GC pressure due to allocating a new array per frame. Extending the optional `target` parameter pattern to array-based operations prevents these allocations and improves execution time by ~35%.
**Action:** Always provide an optional `target` array parameter for functions that transform arrays of objects. Implement truncation logic (`target.length = Math.min(target.length, count)`) to ensure stale data from previously larger target arrays is discarded correctly.
