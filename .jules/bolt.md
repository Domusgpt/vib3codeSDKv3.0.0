...
## 2024-06-25 - Array Iteration Performance in Render Loops
**Learning:** Using `.forEach()` inside hot methods like rendering loops or high-frequency parameter updates (`RealHolographicSystem` and `QuantumEngine`) causes excessive closure allocations per frame, leading to GC pressure and frame drops.
**Action:** Always replace `Array.prototype.forEach` with standard `for` loops (`for (let i = 0; i < len; i++)`) caching the length, when iterating in animation or render loops.
