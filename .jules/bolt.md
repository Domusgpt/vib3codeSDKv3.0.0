...
## 2024-05-26 - [Iterator Optimization in Render Loops]
**Learning:** Iterating over arrays and objects with `.forEach` and `Object.keys(obj).forEach` allocates closures and creates GC pressure when invoked per-frame in `requestAnimationFrame` loops (e.g., `RealHolographicSystem._renderDirectFrame`). Standard `for` and `for...in` loops completely eliminate this allocation, offering significantly higher iterations-per-second, and most importantly preventing micro-stutters during rendering.
**Action:** Always replace `.forEach`, `Object.keys`, and `Object.entries` inside high-frequency rendering and update loops with standard `for`, `for...in` (with `hasOwnProperty` check), or `for...of` loops. Measure object property iteration to verify standard iteration bypasses internal array generation.
