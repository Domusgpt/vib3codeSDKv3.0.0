...
## 2024-05-25 - [Object and Array Iteration Optimization]
**Learning:** In high-frequency methods like render loops and parameter updates, `.forEach()` and `Object.entries(obj).forEach()` cause substantial closure allocations and GC pressure. Replacing them with standard `for` loops (caching length) and `for...in` loops (with `hasOwnProperty`) improves iteration performance significantly while completely eliminating per-frame closure allocations.
**Action:** Always prefer standard `for` loops for array iteration and `for...in` with `Object.prototype.hasOwnProperty.call()` for object property iteration in hot paths like render loops. Ensure loop indices are correctly scoped and loop body variables are `const` or `let` to maintain block scope semantics without closures.
