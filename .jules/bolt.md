...
## 2024-08-07 - [Optimizing Object Iteration]
**Learning:** In `src/core/ParameterMapper.js`, translating parameters using `for...in` loops combined with `hasOwnProperty` instead of `Object.entries().forEach()` or `Object.keys().forEach()` yields significant performance gains (~35-80%) by avoiding intermediate array allocations during hot loop iterations.
**Action:** When working in high-frequency paths (like render loops or parameter updates), prefer standard `for...in` and `for` loops over functional iteration methods (`forEach`, `Object.keys()`, `Object.entries()`) to reduce garbage collection pressure.
