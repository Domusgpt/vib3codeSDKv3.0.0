...
## 2024-05-24 - Hoist Invariant Geometry Structural Calculations
**Learning:** In mathematical generators like `HypertetraCore.js`, static geometrical properties such as pentatope edges, face definitions, or tetrahedral cell centers are entirely constant for a given core geometry size. Calculating these inside a `.map()` or `for` loop over thousands of vertices redundantly generates intermediate objects (`new Vec4()`, arrays, etc.) and executes heavy vector math multiple times.
**Action:** When warping or iterating over vertices in `geometry/warp/` or similar high-frequency geometry generation paths, always pre-calculate structure-invariant data (edge vectors, edge squared lengths, cell centers) *outside* the primary vertex loop, pre-allocating an array for it. Coupled with replacing `.map()` with a standard `for` loop and using `distanceToSquared`, this drastically reduces GC pressure and execution time (e.g. from ~220ms to ~59ms in `warpToEdges`).
