...
## 2024-08-06 - Unrolling Math Equality and Bypassing Identity Allocation
**Learning:** Checking object identity and performing equality comparisons are highly frequent operations in `Mat4x4` and `Rotor4D`. Relying on factory instantiation methods like `Mat4x4.identity()` during a frequent check like `isIdentity()` creates an anti-pattern that allocates objects per invocation on hot paths. In addition, iterating through a 16-element array inside `equals` involves loop overhead that becomes significant over millions of iterations. Furthermore, aggregating L1 norms for mathematical equality verification incorrectly masks localized mismatches for floating points.
**Action:** Unroll loops for array data comparisons in `equals()` and utilize short-circuit evaluation instead of aggregating metrics to minimize overhead. Bypass creating fresh instances during `isIdentity()` checks by validating properties manually against constants.
