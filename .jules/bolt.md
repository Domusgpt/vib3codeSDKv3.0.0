...
**Learning:** Using `.map()` and creating new `Vec4` instances inside hot loops for geometric warp functions (like `warpRadial`, `warpTetrahedral`) creates substantial GC pressure, especially when manipulating large vertex arrays during render loops.
**Action:** Always provide an optional `target` array parameter for geometric iteration functions. Inside the loop, use a fallback assignment (`target[i] = target[i] || new Vec4()`) to enable zero-allocation patterns while maintaining backward compatibility for callers that don't pre-allocate targets.
