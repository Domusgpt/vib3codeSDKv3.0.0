...
## 2024-05-26 - [Avoid closures in render loops]
**Learning:** Using `.forEach()` in per-frame render loops (e.g. `QuantumEngine`, `RealHolographicSystem`) creates closure allocations and function call overhead every frame, leading to GC pressure.
**Action:** Always use standard `for` loops with cached length (or `for...of` if appropriate) for array iteration in high-frequency paths like render loops.
