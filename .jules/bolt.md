...
## 2024-05-18 - Zero-allocation array mapping in math functions
**Learning:** In tight geometric or mathematical loops, relying on `Array.prototype.map()` (e.g., `vectors.map(v => ...)`) generates significant GC pressure due to the allocation of both intermediate arrays and new vector/object instances per iteration.
**Action:** When working on array-level transforms (like `multiplyVec4Array`, `stereographicArray`, `orthographicArray`), replace `.map()` with a standard `for` loop that accepts an optional pre-allocated `target` array to write results in-place, drastically reducing GC pauses in rendering loops.
