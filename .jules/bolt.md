...
## 2024-05-19 - Zero-Allocation Vector Array Transformations
**Learning:** Using `Array.prototype.map()` inside hot rendering or transformation loops creates high garbage collection pressure by constantly allocating new objects (temporary arrays and vectors). `Projection.perspectiveArray` already avoided this pattern, but `multiplyVec4Array`, `stereographicArray`, and `orthographicArray` were still using `.map()`.
**Action:** Always provide an optional `target` array parameter for array transformations, iterating with a traditional `for` loop and falling back to `.map()` only when a pre-allocated target array is not provided. Implement fallback assignment logic (e.g., `target[i] || (target[i] = new Vec4())`) to support zero-allocation caching. Include `@performance` JSDoc comments to document these optimizations clearly.
