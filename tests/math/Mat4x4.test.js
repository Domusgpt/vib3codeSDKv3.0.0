...
describe('Mat4x4', () => {
    ...
    describe('memory optimization', () => {
        it('multiply uses target matrix', () => {
            const a = Mat4x4.translation(1, 0, 0);
            const b = Mat4x4.translation(0, 1, 0);
            const target = new Mat4x4();
            const result = a.multiply(b, target);
            expect(result).toBe(target);
            expect(result.get(0, 3)).toBe(1); // x translation
            expect(result.get(1, 3)).toBe(1); // y translation
        });

        it('multiply handles aliasing with target=this', () => {
            const a = Mat4x4.translation(1, 0, 0);
            const b = Mat4x4.translation(0, 1, 0);
            // a = a * b
            const result = a.multiply(b, a);
            expect(result).toBe(a);
            expect(a.get(0, 3)).toBe(1);
            expect(a.get(1, 3)).toBe(1);
        });

        it('multiplyVec4 uses target vector', () => {
            const m = Mat4x4.uniformScale(2);
            const v = new Vec4(1, 2, 3, 4);
        ...
        it('multiplyVec4 handles aliasing with target=v', () => {
            const m = Mat4x4.rotationXY(Math.PI / 2); // Rotate 90 deg in XY
            // (1, 0, 0, 0) -> (0, 1, 0, 0)
        ...
        it('scale uses target matrix', () => {
            const m = Mat4x4.identity();
            const target = new Mat4x4();
            const result = m.scale(2, target);
            expect(result).toBe(target);
            expect(result.get(0, 0)).toBe(2);
        });
    });
