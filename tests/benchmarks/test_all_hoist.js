...
function runBenchmark() {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
        warpRadial(vertices, 1, 1, target);
    }
    let end = performance.now();
    const radialAlloc = end - start;
    console.log(`warpRadial (Target): ${radialAlloc.toFixed(2)}ms`);
    ...
    for (let i = 0; i < ITERATIONS; i++) {
        warpStereographic(vertices, 1, 1, target);
    }
    end = performance.now();
    const stereoAlloc = end - start;
    console.log(`warpStereographic (Target): ${stereoAlloc.toFixed(2)}ms`);
    ...
    start = performance.now();
    for (let i = 0; i < ITERATIONS; i++) {
        warpHopf(vertices, 1, 1, target);
    }
    end = performance.now();
    const hopfAlloc = end - start;
    console.log(`warpHopf (Target): ${hopfAlloc.toFixed(2)}ms`);
    ...
    start = performance.now();
    for (let i = 0; i < ITERATIONS; i++) {
        warpTetrahedral(vertices, 1, 1, target);
    }
    end = performance.now();
    const tetraAlloc = end - start;
    console.log(`warpTetrahedral (Target): ${tetraAlloc.toFixed(2)}ms`);
    ...
    start = performance.now();
    for (let i = 0; i < ITERATIONS; i++) {
        warpToEdges(vertices, 1, 0.5, target);
    }
    end = performance.now();
    const edgesAlloc = end - start;
    console.log(`warpToEdges (Target): ${edgesAlloc.toFixed(2)}ms`);
    ...
    start = performance.now();
    for (let i = 0; i < ITERATIONS; i++) {
        warpToCells(vertices, 1, 0.7, target);
    }
    end = performance.now();
    const cellsAlloc = end - start;
    console.log(`warpToCells (Target): ${cellsAlloc.toFixed(2)}ms`);
}
