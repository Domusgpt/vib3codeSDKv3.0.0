...
optimizedScene.findNodesInBoxOpt = function(min, max) {
    ...
    this.root.traverse(node => {
        ...
        const px = wm[12];
        if (px < minX || px > maxX) return;
        const py = wm[13];
        if (py < minY || py > maxY) return;
        const pz = wm[14];
        if (pz < minZ || pz > maxZ) return;
        const pw = wm[15];
        if (pw < minW || pw > maxW) return;
...
console.time('findNodesInBoxOpt (inline)');
for (let i = 0; i < 5000; i++) {
    optimizedScene.findNodesInBoxOpt(min, max);
}
console.timeEnd('findNodesInBoxOpt (inline)');
