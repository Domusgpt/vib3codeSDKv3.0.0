...
optimizedScene.raycastOpt = function(origin, direction, maxDistance = 1000) {
    ...
    this.root.traverse(node => {
        ...
        // 1. Extract world pos from matrix directly instead of allocating new Vec4
        const wm = node.worldMatrix.data;
        const nx = wm[12], ny = wm[13], nz = wm[14], nw = wm[15];

        // 2. Inline: toNode = node.pos - origin
        const tnx = nx - ox;
        const tny = ny - oy;
        const tnz = nz - oz;
        const tnw = nw - ow;

        // 3. Inline: dist = toNode.dot(dir)
        const dist = tnx*dx + tny*dy + tnz*dz + tnw*dw;

        if (dist > 0 && dist < maxDistance) {
            // 4. Inline: closest = origin + dir * dist
            const cx = ox + dx * dist;
            const cy = oy + dy * dist;
            const cz = oz + dz * dist;
            const cw = ow + dw * dist;

            // 5. Inline: perpDistSq = lengthSquared(node.pos - closest)
            const pdx = nx - cx;
            const pdy = ny - cy;
            const pdz = nz - cz;
            const pdw = nw - cw;
            ...
            // Compare squared distance (0.5^2 = 0.25)
            if (perpDistSq < 0.25) {
