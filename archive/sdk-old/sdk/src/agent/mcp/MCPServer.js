...
export class MCPServer {
    ...
    configureAudioBand(args) {
        ...
        // Update via global audio config if available (browser only)
        const isBrowser = typeof window !== 'undefined';
    ...
    exportPackage(args) {
        ...
        if (includeReactivity && reactivityConfig) {
            package_.reactivity = reactivityConfig;
        }
        ...
        return {
            package: package_,
            format,
            suggested_next_actions: ['save_to_gallery', 'apply_behavior_preset']
        };
    }
     ...
     */
    listBehaviorPresets() {
        const presets = Object.entries(MCPServer.BEHAVIOR_PRESETS).map(([key, value]) => ({
            id: key,
            name: value.name,
