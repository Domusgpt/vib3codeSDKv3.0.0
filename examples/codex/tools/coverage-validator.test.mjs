...
describe('resolveCoverageJsonPath', () => {
  ...
  it('rejects unsafe absolute and traversal paths', () => {
    expect(resolveCoverageJsonPath('/tmp/gold-standard-coverage.json')).toBeNull();
    expect(resolveCoverageJsonPath('../synesthesia/gold-standard-coverage.json')).toBeNull();
    expect(resolveCoverageJsonPath('synesthesia/../../secret.json')).toBeNull();
  });
});
