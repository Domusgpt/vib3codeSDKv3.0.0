...
- Header actions provide direct paths to codex, notes, MCP labs, and raw site runtime.
- A load-status bar exposes runtime load/reload state for quick observability in constrained environments.
...
- QA protocol: `QA_PROTOCOL.md`
- MCP labs: `mcp-labs/`

## Transitional Acceptance Criteria

1. Entry discoverable from codex index with active status and working run/read links.
2. Narrative runtime reachable from codex entry shell.
3. Coverage matrix and QA protocol present for agent traceability.
4. At least one MCP lab path documented for controlled reproduction.
