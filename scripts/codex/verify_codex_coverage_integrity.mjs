...
} from '../../examples/codex/tools/coverage-validator.mjs';
...
for (const entry of entries) {
  ...
  const jsonCoveragePath = resolveCoverageJsonPath(entry.coverage_href);

  if (!jsonCoveragePath) {
    console.error(`[${slug}] invalid coverage_href: expected .json or .md path`);
    failures += 1;
    continue;
  }
  ...
  try {
    ...
    console.error(`[${slug}] unable to load ${coverageFile}: ${error.message}`);
    failures += 1;
    continue;
  }
  ...
  console.log(`[${slug}] coverage OK (${document.coverage.length} records)`);
}

if (checked === 0) {
  console.error('no active coverage files discovered in catalog entries');
  process.exit(1);
}
