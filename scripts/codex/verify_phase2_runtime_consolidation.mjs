...
const canonicalImportPolicy = {
  forbid: [/from '\.\.\/\.\.\/src\//],
};

function checkCanonicalImportPolicy(file) {
  const full = path.join(root, file);
  const content = fs.readFileSync(full, 'utf8');
  for (const pattern of canonicalImportPolicy.forbid) {
    if (pattern.test(content)) {
      console.error(`canonical module has forbidden import prefix in ${file}: ${pattern}`);
      return 1;
    }
  }
  return 0;
...
for (const file of canonicalFiles) {
  ...
  failed += checkCanonicalImportPolicy(file);
}
...
console.log(`Phase 2 verification passed: ${canonicalFiles.length} canonical files and ${wrapperExpectations.length} wrappers validated with import policy checks.`);
