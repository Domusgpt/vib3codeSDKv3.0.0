...
function ensureHrefUnderSlug(ctx, slug, label, href) {
  if (!isSafeRelativeHref(href)) {
    console.error(`${ctx} invalid ${label}: must be a safe relative path (${href})`);
    return 1;
  }

  const cleanHref = path.posix.normalize(String(href).trim());
  if (!cleanHref.startsWith(`${slug}/`)) {
    console.error(`${ctx} invalid ${label}: must stay under ${slug}/ (${href})`);
    return 1;
  }
...
for (const entry of entries) {
  ...
  failures += ensureHrefUnderSlug(ctx, entry.slug, 'read_href', entry.read_href);
  failures += ensureHrefUnderSlug(ctx, entry.slug, 'run_href', entry.run_href);
  failures += ensureHrefUnderSlug(ctx, entry.slug, 'coverage_href', entry.coverage_href);
  ...
  if (!entry.mcp_labs) {
    ...
    failures += ensureHrefUnderSlug(ctx, entry.slug, 'mcp_labs', entry.mcp_labs);
