...
export class ParameterMapper {
    ...
    fromUnified(params, targetSystem) {
        ...
        const targetMappingTo = this.mappings[targetSystem]?.to;
        const targetMappingFrom = this.mappings[targetSystem]?.from;
        ...
        for (const key in params) {
            if (Object.prototype.hasOwnProperty.call(params, key)) {
                ...
                if (targetMappingTo?.[systemKey] !== undefined ||
                    targetMappingFrom?.[key] !== undefined) {
                    systemParams[systemKey] = params[key];
                }
            }
        }
    ...
    getDefaults(system) {
        ...
        const targetMappingTo = this.mappings[system]?.to;
        const targetMappingFrom = this.mappings[system]?.from;

        for (const unifiedKey in this.unifiedSchema) {
            if (Object.prototype.hasOwnProperty.call(this.unifiedSchema, unifiedKey)) {
                ...
                if (targetMappingTo?.[systemKey] !== undefined ||
                    targetMappingFrom?.[unifiedKey] !== undefined) {
