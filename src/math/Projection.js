...
export class Projection {
     ...
     * @param {Vec4[]} vectors
     * @param {object|Vec4[]} [options] - Projection options or target array
     * @param {Vec4[]} [target] - Optional target array to write results to for zero-allocation
     * @returns {Vec4[]}
     */
    static stereographicArray(vectors, options = {}, target = null) {
        // Handle options overload for target
        ...
        // Optimization: Zero-allocation iteration when target is provided
        const count = vectors.length;
        for (let i = 0; i < count; i++) {
            const out = target[i];
            if (out) {
