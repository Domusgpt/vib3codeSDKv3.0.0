...
export class Mat4x4 {
     ...
     * @performance Uses unrolled loop for faster property comparisons
     * @param {Mat4x4} m
     ...
     * @performance Optimized to avoid new object allocation from Mat4x4.identity()
     * @param {number} epsilon
     * @returns {boolean}
     */
    isIdentity(epsilon = 1e-6) {
        ...
        return Math.abs(d[0] - 1) <= epsilon &&
               ...
               Math.abs(d[2]) <= epsilon &&
               Math.abs(d[3]) <= epsilon &&
               Math.abs(d[4]) <= epsilon &&
               Math.abs(d[5] - 1) <= epsilon &&
               Math.abs(d[6]) <= epsilon &&
               Math.abs(d[7]) <= epsilon &&
               Math.abs(d[8]) <= epsilon &&
               Math.abs(d[9]) <= epsilon &&
               Math.abs(d[10] - 1) <= epsilon &&
               Math.abs(d[11]) <= epsilon &&
               Math.abs(d[12]) <= epsilon &&
               Math.abs(d[13]) <= epsilon &&
               Math.abs(d[14]) <= epsilon &&
