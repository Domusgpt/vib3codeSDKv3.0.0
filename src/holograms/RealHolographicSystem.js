...
export class RealHolographicSystem {
    ...
    updateParameter(param, value) {
        ...
        // ⚡ Bolt: Zero-allocation for loop replaces .forEach
    ...
    init(context = {}) {
        ...
        if (canvasEl) {
            ...
            // ⚡ Bolt: Zero-allocation loop for hot path cleanup
            for (let i = 0, len = this.visualizers.length; i < len; i++) {
    ...
    render(frameState = {}) {
        ...
        if (this._renderMode === 'bridge') {
            ...
            // ⚡ Bolt: High-frequency render loop uses zero-allocation iteration
            for (let i = 0, len = this.visualizers.length; i < len; i++) {
                const visualizer = this.visualizers[i];
